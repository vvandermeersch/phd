...
read_mean_outputvalue <- function(grid, lambda, output_folder = 'D:/applications/capsis4/var', var = "BiomassOfReserves"){
  ...
  for(l in 1:lambda){
    ...
    filenames <- sapply(grid, function(i){
      ind <- paste0("ind", l, "_")
      file.path(output_folder, paste0(format(i, scientific = FALSE), "_", ind, "yearlyResults.log"))})
