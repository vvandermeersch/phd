...
int mcInit (char *paramFile)
{
  ...
  */
  ...
  while (fgets (line, 1024, fp) != NULL)
  {
	...
	/* dispDist_LDD & dispKernel_LDD, add by VV */
    else if (strcmp (param, "dispDist_LDD") == 0)
    {
      if ((sscanf (line, "dispDist_LDD %d", &dispDist_LDD) != 1) ||
	  (dispDist_LDD < 1))
      {
	status = -1;
	Rprintf ("Invalid dispersal distance on line %d in parameter file %s\n",
	...
	goto End_of_Routine;
      ...
      lineNr++;
      dispKernel_LDD = (double *)malloc (dispDist_LDD * sizeof (double));
      if (fscanf (fp, "dispKernel_LDD %f", &p) != 1)
      {
	status = -1;
	Rprintf ("Dispersal kernel LDD expected on line %d in parameter file %s\n",
		 line, paramFile);
	goto End_of_Routine;
      }
      dispKernel_LDD[0] = p;
      for (i = 1; i < dispDist_LDD; i++)
      {
	if (fscanf (fp, "%f", &p) != 1)
	{
	  status = -1;
