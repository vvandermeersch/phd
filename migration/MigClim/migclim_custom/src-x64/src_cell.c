...
**
** Wim Hordijk    Last modified: 21 September 2022 by VV
**
...
// Same as above, for LDD events
bool mcSrcCell_LDD (int i, int j, int **curState, int **pxlAge, int loopID,
		int habSuit, int **barriers)
{
  int    k, l, realDist, pxlSizeFactor;
  double probCol, rnd;
  ...
  ** sink cell. k and l are the coordinates of the potential source cell.
  */
  for (k = i - dispDist_LDD; k <= i + dispDist_LDD; k++)
  {
    for (l = j - dispDist_LDD; l <= j + dispDist_LDD; l++)
    {
      /*
      ** 1. Test of basic conditions to see if a pixel could be a potential
      **    source cell:
	    ...
	    (curState[k][l] != loopID))
	{
	  if (pxlAge[k][l] >= iniMatAge)
	  {
	    ...
	    */
	    realDist = (int)round (sqrt ((k-i)*(k-i) + (l-j)*(l-j)));
	    if ((realDist > 0) && (realDist <= dispDist_LDD))
	    {
	      /*
	      ** 3. Compute the probability of colonization of the sink pixel.
	      **    This probability depends on several factors:
	      ...
	      */
	      if (pxlAge[k][l] >= fullMatAge)
	      {
			probCol = dispKernel_LDD[realDist-1] * pxlSizeFactor * (habSuit / 1000.0);
	      }
	      else
	      {
			probCol = dispKernel_LDD[realDist-1] * pxlSizeFactor * propaguleProd[pxlAge[k][l] - iniMatAge] * (habSuit / 1000.0); // habitat suit not taken into account 
	      }
	      rnd = UNIF01;
	      if (rnd < probCol || probCol == 1.0)
	      {
