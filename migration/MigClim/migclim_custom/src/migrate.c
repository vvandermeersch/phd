...
** Wim Hordijk & Robin Engler:  Last modified: 21 September 2022 by VV
*/
...
*/
int     nrRows, nrCols, envChgSteps, dispSteps, dispDist, dispDist_LDD, iniMatAge, /* dispDist_LDD, add by VV */
        fullMatAge, rcThreshold, barrierType, lddMinDist, lddMaxDist,
        replicateNb;
double *dispKernel, *dispKernel_LDD, *propaguleProd, lddFreq; /* dispKernel_LDD, add by VV */
...
void mcMigrate (char **paramFile, int *nrFiles)
{
  ...
  bool    advOutput, habIsSuitable, cellInDispDist, cellInDispDist_LDD, tempResilience;
      ...
      ** potentially suitable habitats */
      ...
      for(dispStep = 1; dispStep <= dispSteps; dispStep++){
	    ...
	    ** Loop through the cellular automaton. */
	    for(i = 0; i < nrRows; i++){
	      for(j = 0; j < nrCols; j++){
	        ...
	        **    function). */
			...
			/* If sink cell has not been colonised, try LDD event , add by VV */
			if(habIsSuitable && !cellInDispDist && lddFreq > 0.0){
				
				/* Set the probability of generating an LDD event. This
	    	      ** probability is weighted by the age of the cell. VV : should we really weight the prob ?! */
	    	    ...
	    	    if(UNIF01 <= lddSeedProb || lddSeedProb == 1.0){
					...
					// Rprintf ("LDD event may happens! \n");
					
					/* Now we search if there is a suitable source cell to colonize the sink cell. */
					if (mcSrcCell_LDD (i, j, currentState, pixelAge, loopID, habSuitability[i][j], barriers)) cellInDispDist_LDD = true;
				
					/* Update pixel status. */
					if(habIsSuitable && cellInDispDist_LDD){
						
					  Rprintf ("Cell colonised by LDD event! \n");
	    ...
	    /* If the LDD frequence is larger than zero, perform it. commented by V.V. */
	    // if(lddFreq > 0.0){
