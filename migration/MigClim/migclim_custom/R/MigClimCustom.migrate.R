                             ...
                             testMode=FALSE, fullOutput=FALSE, keepTempFiles=FALSE)
{
  ...
  if(is.character(iniDist)){
	  ...
	  if (substr(barrier, nchar(barrier)-3, nchar(barrier)) == ".asc") barrier <- strtrim(barrier, nchar(barrier)-4)
	  if (substr(barrier, nchar(barrier)-3, nchar(barrier)) == ".tif") barrier <- strtrim(barrier, nchar(barrier)-4)
  ...
  if(is.data.frame(iniDist)){
	  ...
	  if(file.exists(iniDist)){
	    ...
	    rm(Rst)
	  } else stop(paste("The 'iniDist' raster could not be found. Make sure the file ", getwd(), "/", iniDist, " exists and that its path is correct.\n", sep=""))
  }
  if(is.na(RExt)) stop("Input data not recognized. The 'iniDist' input raster data must be in one of the following formats: ascii grid (.asc), geoTiff (.tif), ESRI grid or R raster (no extension).\n")
  ...
  if(RExt==".DataFrame"){
	...
	if(is.vector(barrier) && !is.character(barrier)) barrier <- as.data.frame(barrier)
  ...
  if(!identical(barrier, "")) if(!file.exists(paste(barrier,RExt,sep=""))){
	
	stop(paste("The 'barrier' file '", barrier, RExt, "' could not be found.\n", sep=""))
  ...
  if(lddFreq > 0.0){
    ...
    write(paste("dispDist_LDD", length(dispKernel_LDD)), file=fileName, append=T)
	write(c("dispKernel_LDD", dispKernel_LDD), file=fileName, append=T, ncolumns=length(dispKernel)+1)
  }
