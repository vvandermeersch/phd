...
# custom version by V. Van der Meersch: deleted a check step (too looooong with large raster)
# added some messages, and change the max. number of dispSteps

MigClim.migrate.custom <- function (iniDist="InitialDist", hsMap="HSmap", rcThreshold=0, 
                             ...
                             testMode=FALSE, fullOutput=FALSE, keepTempFiles=FALSE)
{
  ...
  if(!is.numeric(dispSteps)) stop("'dispSteps' must be a number in the range [1:200]. \n")
  ...
  #
  cat("Check structure of the file... \n")  
