...
wt <- ifelse(model_data$pres == 1, 1, prNum / bgNum)
...
# evaluation
## prediction
prediction_cal <- predict(mod_brt, 
                          model_data, 
                          n.trees = mod_brt$gbm.call$best.trees, 
                          type = "response")
prediction_tot <- predict(mod_brt, 
                          testing_data, 
                          n.trees = mod_brt$gbm.call$best.trees, 
                          type = "response")

europe_prediction <- predict(mod_brt, 
                          europe_data, 
                          n.trees = mod_brt$gbm.call$best.trees, 
                          type = "response")
...
saveRDS(outfile, file = paste0(wd, "/brt/fit/", sp_name, "/brt_", cov_type,"_", Sys.Date(), ".rds"))
