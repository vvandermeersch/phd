###########################################################################
# Script to perform a down-sampled RF with presence/(pseudo-)absence data #
###########################################################################
...
# model
t0 <- proc.time()
mod_rf <- randomForest(pres ~ ., data = model_data, ntree = 2000)
...
saveRDS(outfile, file = paste0(wd, "/random_forest/fit/", sp_name, "/random_forest_pa_", cov_type,"_", Sys.Date(), ".rds"))
