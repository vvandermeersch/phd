...
for(r in regions){
  ...
  for(s in species){
    ...
    if(file.exists(paste0(outdir, "/", s, "_glm_iwlr.csv"))) next
    ...
    # 
    mod_glm <- NULL
    # calculating the case weights (IWLR)
    upwt <- (10^6)^(1 - model_data$occ)
    
    ptm <- proc.time()
    set.seed(myseed)
    # start modeling! We use the "try" notation so if a species fails to fit, the loop will continue.
    lm1 <- glm(occ ~., data = model_data, weights = upwt, family = binomial(link = "logit"))
    if(inherits(try(
      mod_glm <- gam::step.Gam(lm1, 
                               scope = myscope[[r]], 
                               direction = "both", 
                               data = model_data,
                               trace = FALSE)
    ...
    out_file$region <- r
    out_file$model <- "GLM-IWLR"
    out_file$prediction <- predict(mod_glm, evaluation, type = "response")
    out_file$time <- t[3]
    
    write.csv(out_file, sprintf("%s/%s_glm_iwlr.csv", outdir, s), row.names = FALSE)
    print(n)
  }
}
