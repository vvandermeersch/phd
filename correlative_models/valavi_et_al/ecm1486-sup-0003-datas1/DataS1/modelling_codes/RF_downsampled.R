...
outdir <- "models_output/rf_downsampled"
...
for(r in regions){
  ...
  for(s in species){
    ...
    if(file.exists(paste0(outdir, "/", s, "_rf_downsampled.csv"))) next
    ...
    out_file$region <- r
    out_file$model <- "RF down-sampled"
    out_file$prediction <- as.numeric(predict(mod_rf, evaluation, type = "prob")[,"1"])
    out_file$time <- t[3]

    write.csv(out_file, sprintf("%s/%s_rf_downsampled.csv", outdir, s), row.names = FALSE)
    print(n)
  }
}
