...
for(r in regions){
  ...
  for(s in species){
    ...
    if(file.exists(paste0(outdir, "/", s, "_mars.csv"))) next
    ...
    k <- ifelse(sum(model_data$occ) < 10, 2, 5)
    # convert the response to factor for MARS model to return probabilities
    model_data$occ <- as.factor(model_data$occ)
    levels(model_data$occ) <- c("C0", "C1")
    ...
    ptm <- proc.time()
    cluster <- makeCluster(6, type = "FORK") # use 6 cores for parallel processing
    ...
    out_file$region <- r
    out_file$model <- "MARS unweighted"
    out_file$prediction <- predict(mod_mars, evaluation[, covars[[r]]], type = "prob")[,"C1"] 
    out_file$time <- t[3]
    
    write.csv(out_file, sprintf("%s/%s_mars.csv", outdir, s), row.names = FALSE)
    print(n)
  }
}
