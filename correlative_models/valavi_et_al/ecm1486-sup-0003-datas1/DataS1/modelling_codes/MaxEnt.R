...
maxFile <- "modelling_codes/maxent_files"
...
for(r in regions){
  ...
  for(s in species){
    ...
    if(file.exists(paste0(outdir, "/", s, "_maxent.csv"))) next
    ...
    out_file$region <- r
    out_file$model <- "MaxEnt"
    out_file$prediction <- predict(mod_maxent, evaluation, args = c("outputformat=cloglog"))
    out_file$time <- t[3]
    
    write.csv(out_file, sprintf("%s/%s_maxent.csv", outdir, s), row.names = FALSE)
    print(n)
  }
}
