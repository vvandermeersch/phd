...
for(r in regions){
  ...
  for(s in species){
    ...
    if(file.exists(paste0(outdir, "/", s, "_lasso.csv"))) next
    ...
    # start modeling! We use the "try" notation so if a species fails to fit, the loop will continue.
    if(inherits(try(
      mod_lasso <- glmnet::cv.glmnet(x = training_sparse,
                                     y = model_data$occ,
                                     family = "binomial",
                                     alpha = 1,
                                     weights = wt,
    ...
    out_file$region <- r
    out_file$model <- "Lasso"
    out_file$prediction <- as.numeric(predict(mod_lasso, testing_sparse, type = "response", s = "lambda.min"))
    out_file$time <- t[3]

    write.csv(out_file, sprintf("%s/%s_lasso.csv", outdir, s), row.names = FALSE)
    print(n)
  }
}
