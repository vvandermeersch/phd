...
for(r in regions){
  ...
  for(s in species){
    ...
    if(file.exists(paste0(outdir, "/", s, "_svmw.csv"))) next
    ...
    t <- proc.time() - ptm
    # modPred <- predict(mod_svmw, evaluation, probability = TRUE)
    ...
    out_file$region <- r
    out_file$model <- "SVM weighted"
    # out_file$prediction <- attr(modPred, "probabilities")[,"1"]
    out_file$prediction <- predict(mod_svmw, evaluation, probability = TRUE)
    out_file$time <- t[3]

    write.csv(out_file, sprintf("%s/%s_svmw.csv", outdir, s), row.names = FALSE)
    print(n)
  }
}
