...
for(r in regions){
  ...
  for(s in species){
    ...
    if(file.exists(paste0(outdir, "/", s, "_cforest.csv"))) next
    ...
    out_file$region <- r
    out_file$model <- "cforest"
    out_file$prediction <- purrr::map_dbl(cfpred, purrr::pluck(2))
    out_file$time <- t[3]
    
    write.csv(out_file, sprintf("%s/%s_cforest.csv", outdir, s), row.names = FALSE)
    print(n)
  }
}
