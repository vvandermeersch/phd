...
for(r in regions){
  ...
  for(s in species){
    ...
    if(file.exists(paste0(outdir, "/", s, "_glm.csv"))) next
    ...
    set.seed(myseed)
    # start modeling! We use the "try" notation so if a species fails to fit, the loop will continue.
    lm1 <- glm(occ ~., data = model_data, weights = wt, family = binomial(link = "logit"))
    if(inherits(try(
    ...
    write.csv(out_file, sprintf("%s/%s_glm.csv", outdir, s), row.names = FALSE)
    print(n)
  }
}
