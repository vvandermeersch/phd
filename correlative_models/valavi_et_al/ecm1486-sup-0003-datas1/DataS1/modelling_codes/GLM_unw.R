...
for(r in regions){
  ...
  for(s in species){
    ...
    if(file.exists(paste0(outdir, "/", s, "_glm_unw.csv"))) next
    ...
    ptm <- proc.time()
    set.seed(myseed)
    lm1 <- glm(occ ~., data = model_data, family = binomial(link = "logit"))
    if(inherits(try(
    ...
    out_file$region <- r
    out_file$model <- "GLM unweighted"
    out_file$prediction <- predict(mod_glm, evaluation, type = "response")
    out_file$time <- t[3]
    
    write.csv(out_file, sprintf("%s/%s_glm_unw.csv", outdir, s), row.names = FALSE)
    print(n)
  }
}
