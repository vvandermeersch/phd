...
for(r in regions){
  ...
  for(s in species){
    ...
    if(file.exists(paste0(outdir, "/", s, "_xgboost.csv"))) next
    ...
    # convert the response to factor for XGBoost model to return probabilities
    ...
    out_file$region <- r
    out_file$model <- "XGBoost"
    out_file$prediction <- predict(xgb_fit, evaluation, type = "prob")[,"C1"]
    out_file$time <- t[3]

    write.csv(out_file, sprintf("%s/%s_xgboost.csv", outdir, s), row.names = FALSE)
    print(n)
  }
}
