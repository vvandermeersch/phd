...
for(r in regions){
  ...
  for(s in species){
    ...
    if(file.exists(paste0(outdir, "/", s, "_svm.csv"))) next
    ...
    t <- proc.time() - ptm
    # modPred <- predict(mod_svm, evaluation, probability = TRUE)
    ...
    out_file$prediction <- predict(mod_svm, evaluation, probability = TRUE)
    out_file$time <- t[3]
    
    write.csv(out_file, sprintf("%s/%s_svm.csv", outdir, s), row.names = FALSE)
    print(n)
  }
}
