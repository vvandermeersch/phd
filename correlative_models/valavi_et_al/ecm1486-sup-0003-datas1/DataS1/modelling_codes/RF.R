...
for(r in regions){
  ...
  for(s in species){
    ...
    if(file.exists(paste0(outdir, "/", s, "_rf.csv"))) next
    ...
    set.seed(myseed)
    if(inherits(try(
      mod_rf <- randomForest(occ ~ ., data = model_data, ntree = 500)
    ), "try-error")){
    ...
    write.csv(out_file, sprintf("%s/%s_rf.csv", outdir, s), row.names = FALSE)
    print(n)
  }
}
