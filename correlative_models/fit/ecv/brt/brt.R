...
#----------------------------------------------------#
# Script to perform a BRT as in Valavi et al. (2022) #
#----------------------------------------------------#
...
# 2. Run calibrations
cat("Boosted Regression Tree calibrations\n\n")
...
for(k in seq_len(length(folds))){
  ...
  # run model
  t0 <- proc.time()
  mod_brt <- gbm.step(data = train_data, 
                      gbm.x = 1:6,
                      gbm.y = 7, 
                      family = "bernoulli",
                      site.weights = wt,
  ...
  # predict test set
  test_data$pred <- predict(mod_brt, test_data, n.trees = mod_brt$gbm.call$best.trees, type = "response")
  ...
  sp_data$pred <- predict(mod_brt, sp_data, n.trees = mod_brt$gbm.call$best.trees, type = "response")
  ...
  outfile$name <- "Boosted Regression Tree" # model name
  outfile$modality <- "ecv" # modelling modality 
  outfile$fit_date <- Sys.Date() # date
  outfile$model <- mod_brt # model object
  ...
  # save file
  dir.create(paste0(wd, "/brt/fit/", sp_name), showWarnings = FALSE)
  saveRDS(outfile, file = paste0(wd, "/brt/fit/", sp_name, "/brt_", cov_type, "_fold", k,".rds"))
...
# run model
t0 <- proc.time()
mod_brt <- gbm.step(data = full_data, 
                    gbm.x = 1:6,
                    gbm.y = 7, 
                    family = "bernoulli",
                    site.weights = wt,
