library(ModelMetrics, include.only = c("rmse"))
source(paste0(wd,"/functions/linear_scaling.R"))
source(paste0(wd, "/functions/read_mean_fitness.R"))

rmse_test <- function(x, Yobs, run, run_all, wd, species_files, lambda, param){
  ...
  # Compute RMSE
  rmse <- apply(Ysim, 1, FUN= function(x){ rmse(Yobs, x) })
