...
biblio-style: apsr
title: "PHENOFIT parametrization of *F. sylvatica*"
...
sim_dir <- "D:/simulations/phenofit/forward/fagus_sylvatica/EvolLett2019"

# Species presence points and pres/abs points
load("D:/species/processed/fagus_sylvatica/fagus_sylvatica_occ_rs.Rdata")
load("D:/species/processed/fagus_sylvatica/fagus_sylvatica_presabs.Rdata")

# Load fitness
fitness <- read_mean_outputvalue(sim_dir, "Fitness") # all Europe
fitness_presabs <- read_mean_outputvalue(sim_dir, "Fitness", points = fagussylvatica_presabs) # only on pres/abs points

# AUC on pres/abs points 
auc_presabs <- auc(roc(fitness_presabs, as.factor(fagussylvatica_presabs$pres)))
...
- Species : *Fagus sylvatica* (J. Gauzere parametrization)
...
ggplot() + 
  geom_raster(data = fagussylvatica_presabs[fagussylvatica_presabs$pres==1,], aes(x = lon, y = lat), color = NA, fill = "#5eb88a", alpha=1) +
  geom_raster(data = fagussylvatica_presabs[fagussylvatica_presabs$pres==0,], aes(x = lon, y = lat), color = NA, fill = "#f5cfc6", alpha=0.5) +
