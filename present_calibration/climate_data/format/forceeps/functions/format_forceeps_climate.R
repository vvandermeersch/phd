...
format_forceeps_climate <- function(years, ncells, out_folder, source_folder, ncores, create_files = TRUE){
  ...
  for(yr in years){
    ...
    out <- future_apply(array(1:ncores), 1, function(j){
      ...
      for(i in 1:nlines){
        ...
        pre <- t(phenofit_climate$pre[i,])

        cell_data <- data.frame(m = months,
                                tmean = t(phenofit_climate$tmp[i,]), tmin = t(phenofit_climate$tmn[i,]),
                                tmax = t(phenofit_climate$tmx[i,]), pre = pre,
                                nb_rainy_days = pre > 0
                                )
        
        cell_data_mn <- cell_data %>% 
          group_by(m) %>%
          summarise(tmean = mean(tmean), tmin = mean(tmean), tmax = mean(tmax), pre = mean(pre), nb_rainy_days = sum(nb_rainy_days))
        
        cell_data_mn <- data.frame(yr = rep(yr, 12), cell_data_mn)
        
        cell_data_mn <- round(cell_data_mn, 2)
        
        climate_file <- paste0(out_folder, ncells_j[i], ".climate")
        suppressWarnings(write.table(cell_data_mn, climate_file, 
