# Code to extract Q. evergreen/deciduous pollen occurrences
...
for(i in 1:nrow(IDs)){
  ...
  print(dataID)
  try(source(file.path(wd, "quercus", "pollen_taxa_harmonization.R")))
}
...
for(year in seq(0, 21000, 500)){
  pres_r <- create_species_raster_qilex(year, field = 'Quercus.evg.type', pollen_counts, grid)
