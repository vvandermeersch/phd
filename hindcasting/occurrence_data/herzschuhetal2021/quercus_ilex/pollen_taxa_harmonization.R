  ...
  
  {
    ...
    if(length(new_age$meanAgeBP) > 0){
      ...
      if(length(new_age$meanAgeBP) == length(harmonized_counts_df$depth)){ 
        ...
        # save output file:
        write.table(combine_age_counts_sorted, file = file.path(wd, "quercus_ilex", paste0("output/final_harmonized_pollen_counts_dataset_",dataID,".csv")), fileEncoding="UTF-8", row.names=FALSE, sep="\t")
  ...
  
  {
    ...
    if(length(new_age$meanAgeBP) > 0){
      ...
      if(length(new_age$meanAgeBP) == length(harmonized_percentages_df$depth)){ 
        ...
        # save output file:
        write.table(combine_age_perc_sorted, file = file.path(wd, "quercus_ilex", paste0("output/final_harmonized_pollen_percentages_dataset_",dataID,".csv")), fileEncoding="UTF-8", row.names=FALSE, sep="\t")
