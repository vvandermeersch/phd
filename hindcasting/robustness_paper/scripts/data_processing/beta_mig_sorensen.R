...
range(model_performance_withmig$mig_sorensen)
hist(max(model_performance_withmig$mig_sorensen)-model_performance_withmig$mig_sorensen)

beta.fit <- glmmTMB(formula = mig_sorensen ~ hypervolume_sorensen*type2, family = ordbeta(link = "logit"), 
...
beta.pred <- predict(beta.fit, newdata = newdata, type = "response")
