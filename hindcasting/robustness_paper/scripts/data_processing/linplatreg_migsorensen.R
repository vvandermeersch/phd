...
p_fitpb <- nlsLM(formula = mig_sorensen ~ SSlinp(12000-year, a, b, jp),
                 data = model_performance_withmig[model_performance_withmig$type == "4Fittedprocessbased",])

rsquared_csdm <- round(modelr::rsquare(p_tree, model_performance_withmig[model_performance_withmig$type2 == "1Treebased",]), 2)
...
newdata <- data.frame(year=seq(max(model_performance_withmig$year),0, length.out=100))
...
conf_intervals <- rbind(
  ...
  data.frame(newdata,mig_sorensen=predict(p_fitpb,newdata),
             ...
             type="4Fittedprocessbased",wts=FALSE)
)
