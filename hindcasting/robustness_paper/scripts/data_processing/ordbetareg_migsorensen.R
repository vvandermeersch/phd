...
if(run_model){
  ord_fit_mean <- ordbetareg(formula = 
                             ...
                             true_bounds = c(0,1),
                             silent = 0)
  saveRDS(ord_fit_mean, file.path(wd, "data", "ordered_beta_regression_fit_assoon.rds"))
}else{
  ord_fit_mean <- readRDS(file.path(wd, "data", "ordered_beta_regression_fit_assoon.rds"))
}
...
epbm <- hypothesis(ord_fit_mean, 
           "bsp_migsorensen_miclim_hpv_sorensen.mean + bsp_migsorensen_miclim_hpv_sorensen.mean:type4Expertprocessbased = 0", 
           class = NULL)
fpbm <- hypothesis(ord_fit_mean, 
           "bsp_migsorensen_miclim_hpv_sorensen.mean + bsp_migsorensen_miclim_hpv_sorensen.mean:type3Fittedprocessbased = 0", 
           class = NULL)
