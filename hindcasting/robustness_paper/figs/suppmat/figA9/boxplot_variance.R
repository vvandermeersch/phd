...
model_performance_var <- model_performance_withmig %>%
  group_by(type, year) %>%
  dplyr::summarise(sd_sorensen=sd(sorensen)) %>%
  as.data.frame()

conoverTest_out <- conover.test::conover.test(x=model_performance_var$sd_sorensen, 
...
boxplot_sdperformance <- ggplot(model_performance_var, aes(x = type, y = sd_sorensen, color = type, fill = type)) +
  geom_boxplot(alpha = 0.3, outlier.size = 0.1, width = 0.3) +
  scale_y_continuous(expand = expansion(mult = c(0, 0)),
                     breaks = seq(0,0.4,0.05),
                     name = "Performance (std. dev.)") +
  geom_text(data = data_letters, aes(x = as.character(Group), label = Letter, y = c(rep(0.37,3))), vjust = 0, inherit.aes = F,
  ...
  coord_cartesian(ylim=c(0.1, 0.40), clip = "on") 
