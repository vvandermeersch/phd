...
plotlist <- unlist(lapply(1:nrow(abies_models), function(i){
  plots <- lapply(c(11750, 11000, 8500, 7000, 500), function(y){
    out <- crop(readRDS(file.path(abies_models[i, "simfolder"], paste0(y,"BP.rds"))), ext(-10,30,34,66)) %>%
    ...
    if(y == 11750){
      plot <- plot +
        annotate("text", x = -12.7, y = 50, label = abies_models[i, "name"], color = "black",
