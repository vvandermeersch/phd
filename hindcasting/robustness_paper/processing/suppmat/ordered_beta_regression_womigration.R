...
if(run_model){
  ord_fit_mean <- ordbetareg(formula = 
                             ...
                             true_bounds = c(0,1),
                             silent = 1)
  saveRDS(ord_fit_mean, file.path(wd, "manuscript", "processing/output", "ordered_beta_regression_fit_womigration.rds"))
}else{
  ord_fit_mean <- readRDS(file.path(wd, "manuscript", "processing/output", "ordered_beta_regression_fit_womigration.rds"))
}

# Get conditional effects
# posterior_summary(ord_fit_mean)
# epbm <- hypothesis(ord_fit_mean, 
#            "bsp_sorensen_miclim_hpv_sorensen.mean + bsp_sorensen_miclim_hpv_sorensen.mean:type3Expertprocessbased = 0", 
#            class = NULL)
# fpbm <- hypothesis(ord_fit_mean, 
#            "bsp_sorensen_miclim_hpv_sorensen.mean + bsp_sorensen_miclim_hpv_sorensen.mean:type2Fittedprocessbased = 0", 
#            class = NULL)
