...
if(run_model){
  ord_fit_mean <- ordbetareg(formula = 
                             ...
                             true_bounds = c(0,1),
                             silent = 1)
  saveRDS(ord_fit_mean, file.path(wd, "manuscript", "processing/output", "ordered_beta_regression_fit_from12000.rds"))
}else{
  ord_fit_mean <- readRDS(file.path(wd, "manuscript", "processing/output", "ordered_beta_regression_fit_from12000.rds"))
}
