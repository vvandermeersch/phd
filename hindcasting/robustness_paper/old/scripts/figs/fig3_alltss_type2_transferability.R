...
tss_performance <- ggplot(model_performance_withmig, aes(x = median, y = mig_tss, color = type2, fill = type2)) +
...
annotate("rect", xmin=round(max(model_performance_withmig$median),1),xmax=round(max(model_performance_withmig$median),0), 
  ...
  theme(panel.border = element_blank(), panel.grid.major = element_blank(),
        ...
        axis.title.y = element_text(colour = "black", family= "Helvetica Narrow", size = 8, hjust = 0.22),
...
conoverTest_out <- conover.test::conover.test(x=model_performance_relchg[model_performance_relchg$crit == TRUE,]$rel_chg_tss, 
                                              g=model_performance_relchg[model_performance_relchg$crit == TRUE,]$type2test, 
                                              kw=FALSE,
...
boxplot_under <- ggplot(model_performance_relchg[model_performance_relchg$crit == TRUE,], 
                       aes(x = type2, y = rel_chg_tss, color = type2, fill = type2, group = type2test)) +
  geom_boxplot(alpha = 0.3, outlier.size = 0.1, position = position_dodge2(preserve = "single", width = -2),
               linewidth = 0.3, width = 0.8) +
  scale_y_continuous(expand = expansion(mult = c(0, 0)),
                     breaks = seq(0.6, -1.4, -0.3),
                     name = "Transferability") +
  geom_text(data = data_letters, aes(x = type2, label = Letter, group = Group , y = 0.6), vjust = 0, inherit.aes = F,
            family = "Helvetica Narrow", size = 2.15, position=position_dodge(width=0.8)) +
  geom_text(data = data_letters, aes(x = type2, label = sign, y = c(NA, -0.09, NA, 0.02, NA, NA), group = Group), vjust = 0, inherit.aes = F,
            family = "Helvetica Narrow", size = 1.75, position=position_dodge(width=0.75)) +
  scale_color_manual(breaks= c("1Treebased", "2Regressionbased", '3Expertprocessbased', "4Fittedprocessbased"),
                     values= c( "#457b9d", "#82BCC4", "#e86117","#995D81")) +
  scale_fill_manual(breaks= c("1Treebased", "2Regressionbased", '3Expertprocessbased',  "4Fittedprocessbased"),
                    values= c( "#457b9d", "#82BCC4", "#e86117", "#995D81")) +
  ...
  coord_cartesian(ylim=c(-1.4, 0.8))



# higher than critical point
# dunnTest_out <- FSA::dunnTest(rel_chg_tss ~ type2test, method = "bh", 
#                               data = model_performance_relchg[model_performance_relchg$crit == FALSE,])
conoverTest_out <- conover.test::conover.test(x=model_performance_relchg[model_performance_relchg$crit == FALSE,]$rel_chg_tss, 
                                g=model_performance_relchg[model_performance_relchg$crit == FALSE,]$type2test, 
                                kw=FALSE,
                                method="by")
letters <- rcompanion::cldList(P.adjusted ~ comparisons, data = conoverTest_out, threshold = 0.05)
data_letters <- data.frame(letters)
...
boxplot_over <- ggplot(model_performance_relchg[model_performance_relchg$crit == FALSE,], 
                       aes(x = type2, y = rel_chg_tss, color = type2, fill = type2, group = type2test)) +
  geom_boxplot(alpha = 0.3, outlier.size = 0.1, position = position_dodge2(preserve = "single", width = -2),
               linewidth = 0.3, width = 0.8) +
  scale_y_continuous(expand = expansion(mult = c(0, 0)),
  ...
  geom_text(data = data_letters, aes(x = type2, label = Letter, group = Group , y = 0.6), vjust = 0, inherit.aes = F,
            family = "Helvetica Narrow", size = 2.15, position=position_dodge(width=0.8)) +
  geom_text(data = data_letters, aes(x = type2, label = sign, y = -0.7, group = Group), vjust = 0, inherit.aes = F,
            family = "Helvetica Narrow", size = 1.75, position=position_dodge(width=0.75)) +
  scale_color_manual(breaks= c("1Treebased", "2Regressionbased", '3Expertprocessbased', "4Fittedprocessbased"),
                     values= c( "#457b9d", "#82BCC4", "#e86117","#995D81")) +
  scale_fill_manual(breaks= c("1Treebased", "2Regressionbased", '3Expertprocessbased',  "4Fittedprocessbased"),
...
tss_performance2 <- tss_performance +
  ...
  annotation_custom(ggplotGrob(boxplot_over), xmin = trunc((2-critpoint_rbcsdm)*100)/100-0.02, xmax = 1.89, 
                    ymin = 0.79, ymax = 1.4) +
  annotation_custom(ggplotGrob(boxplot_under), xmin = 0.96, xmax = trunc((2-critpoint_rbcsdm)*100)/100+0.02, 
                    ymin = 0.79, ymax = 1.4) +
