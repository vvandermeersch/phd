...
dunnTest_out <- FSA::dunnTest(mig_sorensen ~ mod, method = "bh", data = model_performance_withmig[
...
qboxplot_under <- ggplot(model_performance_withmig[model_performance_withmig$crit ==TRUE & model_performance_withmig$species == "quercus",], 
                         aes(x = mod, y = mig_sorensen, color = type2, fill = type2)) +
  geom_boxplot(alpha = 0.3, outlier.size = 0.1) +
  scale_y_continuous(expand = expansion(mult = c(0, 0)),
                     breaks = seq(0,1,0.2),
                     name = "Sorensen index") +
  geom_text(data = data_letters, aes(x = as.character(Group), label = Letter, y = 0.95), vjust = 0, inherit.aes = F,
