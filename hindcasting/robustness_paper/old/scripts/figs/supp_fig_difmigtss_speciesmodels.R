...
dunnTest_out <- FSA::dunnTest(mig_tss-tss ~ mod, method = "bh", data = model_performance_withmig[
...
qboxplot_under <- ggplot(model_performance_withmig[model_performance_withmig$crit ==TRUE & model_performance_withmig$species == "quercus",], 
                         aes(x = mod, y = mig_tss-tss, color = type2, fill = type2)) +
  geom_hline(aes(yintercept = 0), col = "darkgrey", linetype = "dotted") +
  geom_boxplot(alpha = 0.3, outlier.size = 0.1) +
  scale_y_continuous(expand = expansion(mult = c(0, 0)),
                     breaks = round(seq(-0.6,0.4, 0.2),1),
                     name = expression(Delta*"TSS due to migration")) +
  geom_text(data = data_letters, aes(x = as.character(Group), label = Letter, y = 0.4), vjust = 0, inherit.aes = F,
  ...
  coord_cartesian(ylim=c(-0.6, 0.4), clip = "off") +
