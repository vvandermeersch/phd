...
p_tree <- nlsLM(formula = mig_sorensen ~ SSlinp(hypervolume_sorensen, a, b, jp), 
                data = model_performance_withmig[model_performance_withmig$type2 == "1Treebased",]) # weights = 1/sd_var^2 ?

p_reg <- nlsLM(formula = mig_sorensen ~ SSlinp(hypervolume_sorensen, a, b, jp), 
               data = model_performance_withmig[model_performance_withmig$type2 == "2Regressionbased",])

p_exppb <- nlsLM(formula = mig_sorensen ~ SSlinp(hypervolume_sorensen, a, b, jp),
                 data = model_performance_withmig[model_performance_withmig$type2 == "3Expertprocessbased",])

p_fitpb <- nlsLM(formula = mig_sorensen ~ SSlinp(hypervolume_sorensen, a, b, jp),
                 data = model_performance_withmig[model_performance_withmig$type2 == "4Fittedprocessbased",])

p_csdm <- nlsLM(formula = mig_sorensen ~ SSlinp(hypervolume_sorensen, a, b, jp), 
                data = model_performance_withmig[model_performance_withmig$type2 == "2Regressionbased" | 
                                                   model_performance_withmig$type2 == "1Treebased",])

p_pb <- nlsLM(formula = mig_sorensen ~ SSlinp(hypervolume_sorensen, a, b, jp), 
              data = model_performance_withmig[model_performance_withmig$type2 == "3Expertprocessbased" | 
...
newdata <- data.frame(hypervolume_sorensen=seq(max(model_performance_withmig$hypervolume_sorensen),0.7, length.out=100))

conf_intervals <- rbind(
  data.frame(newdata,mig_sorensen=predict(p_tree,newdata),
             intconf = predictNLS(p_tree, newdata=newdata, interval="confidence", alpha=0.05, nsim=10000)$summary,
             type2="1Treebased",wts=FALSE),
  data.frame(newdata,mig_sorensen=predict(p_reg,newdata),
