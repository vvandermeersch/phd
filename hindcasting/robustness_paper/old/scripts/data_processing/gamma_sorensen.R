...
min(0.9-model_performance_withmig$mig_tss)
hist(0.9-model_performance_withmig$mig_tss)
hist(1-model_performance_withmig$mig_sorensen)

gamma.fit <- glmmTMB(formula = (1-mig_sorensen) ~ hypervolume_sorensen*type2, family = Gamma(link = "log"), 
                 data = model_performance_withmig)
summary(gamma.fit)


check_gamma_model <- simulateResiduals(fittedModel = gamma.fit, n = 500)
...
gamma.pred <- predict(gamma.fit, newdata = newdata, type = "response", se.fit = TRUE)
pred <- data.frame(hypervolume_sorensen = newdata$hypervolume_sorensen, 
                   type2 = newdata$type2,
                   mig_sorensen = 1-gamma.pred$fit,
                   sd = gamma.pred$se.fit)
...
sorensen_performance <- ggplot(data = pred, aes(x=1-hypervolume_sorensen, y = mig_sorensen, col = type2, fill = type2)) +
  ...
  geom_line() +
  geom_ribbon(aes(ymin = mig_sorensen-sd, ymax = mig_sorensen+sd), 
