...
# this code is largely inspired by J. Kaplan code available on GitHub: https://github.com/ARVE-Research/LPJ-LMfire
# mainly based on a paper by X. Yin (1998)

# adapted by V. Van der Meersch - 12/12/2022 (code was originally writen in C)

# pet_method : "kaplan" or "faoPM"

# main function
compute_glo_radiation_from_toa <- function(toa_file, input_file, gwgen_file, elev_data, output_dir, faoPM = F, faoPM2 = F, ncores = 1){
  ...
  # read data
  toa_data <- fread(toa_file)
  input_data <- fread(input_file)
  gwgen_data <- fread(gwgen_file)
  
  # compute the "precipitation equitability index" (approx tmean = tmin + tmax /2)
  ...
  peqin <- lapply(1:nrow(peqin_data), function(i){calculate_prec_eq_index(peqin_data[i,])})
  ...
  # compute long-wave radiation, needed to compute PET 
  ...
  sw_pet <- future_lapply(1:nrow(toa_data), function(i){
    ...
    if(!faoPM & !faoPM2){
      ...
      while(abs(pet - pet0) > 0.01 & k < 100){
        ...
        # compute global radiation
        sw_i <- calculate_glo_from_toa(toa_data[i,], gwgen_data[i,], pet)
        # compute PET from net radiation (as J. Kaplan)
        netrad <- (1 - albedo) * sw_i - toa_data[i,"lw"]
      ...
      r_nsg <- (1 - albedo) * r_s # net incoming shortwave radiation - water or other evaporative surface with specified Albedo
      r_ng <- r_nsg - r_nl
      ...
      delta <- toa_data[i,"ss"]/1000 #convert to kPa/C
      gamma <- toa_data[i,"gamma"]/1000 #convert to kPa/C
      es <- toa_data[i,"es"]/1000  #convert to kPa
      Tdew <- toa_data[i,"TdewK"]-273.15
      ea <- 0.6108 * exp((17.27 * Tdew)/(Tdew+237.3)) # from FAO
      
      rs_rs0 <- 1-0.29*(gwgen_data[i,"mean_cloud"] + (gwgen_data[i,"mean_cloud"])^2) # Antoine et al, 1996
      
      r_nl <- 4.903e-09 * (0.34 - 0.14 * sqrt(ea)) * 
        ((gwgen_data[i,"tmax"] + 273.2)^4 + (gwgen_data[i,"tmin"] + 273.2)^4)/2 * 
        (1.35 * rs_rs0 - 0.35) # estimated net outgoing longwave radiation, from FAO
      
      netrad <- (1 - albedo) * sw_i/1000 - r_nl
      
      pet <- max((0.408 * delta * netrad + gamma * 900 * u2 * (es - ea) / (gwgen_data[i,"temp"] + 273.15))/
      ...
      while(abs(pet - pet0) > 0.01 & k < 100){
        ...
        # compute global radiation
        sw_i <- calculate_glo_from_toa(toa_data[i,], gwgen_data[i,], pet)
        # compute PET from net radiation (as FAO)
...
calculate_glo_from_toa <- function(toa_data, gwgen_data, pet){
