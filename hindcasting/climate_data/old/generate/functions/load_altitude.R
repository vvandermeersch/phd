...
load_altitude <- function(year, folder, extent){
  ...
  cover_file <- file.path(folder, "tempmin_av_old_sims_1yrAvg_monthly_10minRes_noBias_Europe_24000_0kyr",
                           paste0("tempmin_av_old_sims_1yrAvg_monthly_10minRes_noBias_Europe_", file$name, ".nc"))
...
load_altitude_ICE6GC <- function(year, folder = "D:/climate/ICE-6G-C", folder_hadcm3b = "D:/climate/HadCM3B_60Kyr_Climate/2023_dataset/raw", extent){
  ...
  file <- years_to_file(year)
  years <- file$max:file$min # time goes forward
  cover_file <- file.path(folder_hadcm3b, "tempmin_av_old_sims_1yrAvg_monthly_10minRes_noBias_Europe_24000_0kyr",
                          paste0("tempmin_av_old_sims_1yrAvg_monthly_10minRes_noBias_Europe_", file$name, ".nc"))
  cover <- rast(cover_file, subds = "tempmin_av", lyrs = which(years == year)*12-6)
  cover <- crop(cover, extent)
...
load_albedo <- function(year, folder, extent){
  ...
  file <- years_to_file(year)
  albedo_file <- file.path(folder, "albedos_old_sims_1yrAvg_monthly_10minRes_noBias_Europe_24000_0kyr",
                        paste0("albedos_old_sims_1yrAvg_monthly_10minRes_noBias_Europe_", file$name, ".nc"))
  years <- file$max:file$min # time goes forward
